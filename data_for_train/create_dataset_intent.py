...
if __name__ == '__main__':
    ...
    pos_data = []
    for df_ in dfs[:-1]:
        pos_data.extend(list(zip(df_[['mention', 'attribute_good_ids']].sample(pos_per_class, replace=True).to_dict('records'),
                 df_[['mention', 'attribute_good_ids']].sample(pos_per_class, replace=True).to_dict('records'))))
    pos_data.extend(list(zip(dfs[-1][['mention', 'attribute_good_ids']].sample(pos_size - pos_per_class*(num_attributes-1), replace=True).to_dict('records'),
                      dfs[-1][['mention', 'attribute_good_ids']].sample(pos_size - pos_per_class*(num_attributes-1), replace=True).to_dict('records'))))
    assert len(pos_data) == pos_size

    neg_data = []
    all_attribute_ids = list(all_attribute_ids)
    for att_id in all_attribute_ids[:-1]:
        mentions1 = df_all[df_all['attribute_good_ids'].map(lambda x: att_id in x)][['mention', 'attribute_good_ids']].sample(neg_per_class, replace=True).to_dict('records')
        mentions2 = df_all[df_all['attribute_good_ids'].map(lambda x: att_id not in x)][['mention', 'attribute_good_ids']].sample(neg_per_class, replace=True).to_dict('records')
        neg_data.extend(list(zip(mentions1, mentions2)))
    mentions1 = df_all[df_all['attribute_good_ids'].map(lambda x: all_attribute_ids[-1] in x)][['mention', 'attribute_good_ids']].sample(
        neg_size - neg_per_class*(num_attributes-1), replace=True).to_dict('records')
    mentions2 = df_all[df_all['attribute_good_ids'].map(lambda x: all_attribute_ids[-1] not in x)][['mention', 'attribute_good_ids']].sample(
        neg_size - neg_per_class*(num_attributes-1), replace=True).to_dict('records')
    neg_data.extend(list(zip(mentions1, mentions2)))
    ...
    # y = [1] * len(pos_data) + [1] * len(neg_data)
