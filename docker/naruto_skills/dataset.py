...
class DataWithLabel:
    ...
    def get_data_iterator(self, batch_size=64, num_epochs=1, is_strictly_equal=True):
        ...
        if num_epochs > 0:
            logging.info('There will be %s steps', self.estimate_number_of_steps(batch_size, num_epochs, is_strictly_equal))
    ...
    def show_info(self, is_logging=False):
        ...
        if self.y.shape[0] > 0:
            ...
            if np.isscalar(self.y[0]):
                display_func(pd.DataFrame({'y': self.y})['y'].value_counts())

    def dump(self, path_to_dir, prefix=''):
        if not os.path.isdir(path_to_dir):
            os.mkdir(path_to_dir)
        np.savez(os.path.join(path_to_dir, '{}_{}.npz'.format(prefix, self.name)), X=self.X, y=self.y)
...
class Dataset:
    def __init__(self, data_with_label_train, data_with_label_eval, data_with_label_test):
        self.data_train = data_with_label_train
        self.data_eval = data_with_label_eval
        self.data_test = data_with_label_test

    @staticmethod
    def create_from_entire_data(X, y, eval_prop=0.1, test_prop=0.1):
        """
        :param X:
        :param y:
        :param train_prop:
    ...
    def show_info(self):
        logging.info('Dataset size: %s', len(self.data_train.y) + len(self.data_eval.y) + len(self.data_test.y))
    ...
    def dump(self, path_to_dir):
        ...
        logging.info('Dumped dataset to %s', path_to_dir)
